    ...
    <el-table :data="pagination.records" border>
      ...
      <el-table-column fixed="right" label="操作" width="220">
        <template slot-scope="scope">
          <el-button @click="exportScore(scope.row.examCode)" type="success" size="small">导出成绩</el-button>
...
export default {
  ...
  methods: {
    ...
    exportScore(examCode) {
      if (!examCode) {
        this.$message.error('未找到试卷编号')
        return
      }
      window.location.href = `/api/export/score/${examCode}`
    },
  },
};
</script>
